function [FAConnections,count] = PlotFilamentsAndMembrane_custom03(area1,nth,count,Filaments,Membrane,Adhesions,FAConnections,FH,AH1,AH2,AH3,t,nMono,nAdhes,MemVel,index,TimeVec,ModelParameters)
...
%        end
       ...
       plot(AH1, Adhesions.XYPoints(idx,1), Adhesions.XYPoints(idx,2), '*k', 'MarkerSize', 8)
       
       % plot attached adhesions as red
       idx = FAConnections.AdhesionIndex;
       plot(AH1, Adhesions.XYPoints(idx,1),Adhesions.XYPoints(idx,2),  '*r', 'MarkerSize', 8)
       AD = round( (nAdhes( find(~isnan(nAdhes),1,'last'))/area1) );
       title(AH1,[{['\rmBR-MC    \it\tau_{max}\rm = 3.0 s    \itk_{b}\rm = 10 pN/nm     dt = 0.001 s']},...
...
%                   'FontName','monospaced','FontSize', 14, 'FontWeight','bold')
       ...
       box(AH1, 'on')
       xlabel(AH1,'X(\mum)','FontSize',24,'FontWeight','bold')
       ylabel(AH1,'Y(\mum)','FontSize',24,'FontWeight','bold')
